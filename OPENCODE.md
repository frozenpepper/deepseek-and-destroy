...
orchestrator runs in Claude Code, Codex, OpenCode, or another harness.
...
Persist the resolved absolute path in `state.json`. Reviewer → fixer resume uses the
same DB + recorded session id. Fresh roles get fresh sessions in the same DB.
...
Do not delete the run DB after every task; later reviewer/fixer resume may need it.
...
5. `run_worker.py` owns OpenCode process construction, DB env, title, cwd, log,
   process identity, atomic attempt reservation, optional resume session, report
...
reusing the same attempt/report/log is rejected. `terminal.json` repeats those
bindings so the evidence gate can reject post-launch authority/baseline tampering.
The wrapper writes `attempt.json` after the child exists and `terminal.json` when
that exact child exits.

OpenCode supports `opencode session list --format json`; after exit the wrapper
best-effort resolves the deterministic title to a session id for later resume. A
missing/ambiguous session id does not corrupt task evidence: record the lookup
error and use a fresh fixer when resume cannot be established safely.
...
evidence after FINAL. Discovery of such a writer invalidates the assumption and
enters recovery.
...
python3 <skill-root>/scripts/wait_worker.py \
  --event-dir <attempt-dir> --timeout 3600
```
...
The worker must replace that with `FINAL` at terminal completion. A leftover
skeleton is not a substantive FAIL merely because its safe placeholder says
BLOCKED/FAIL. `evidence_gate.py` routes this to the Evidence Clerk to inspect only
the worker log/exact declared output locations. If exactly one complete FINAL
report from the same attempt exists elsewhere under the run, the Clerk may
canonicalize it byte-for-byte and record source/copy hashes; ambiguity remains
MALFORMED.

## Resume

When a reviewer FAIL is moderate and a trustworthy session id exists, a fixer may
resume that exact session through the same run DB. Heavy review contexts should
still hand findings to a fresh fixer. Fresh re-review is always independent.

`run_worker.py --resume-session <id>` preserves all other transport invariants.
If resume fails immediately for missing session/process state, retry once only when
that exact transient is plausible; otherwise use the configured fresh-fixer path.
