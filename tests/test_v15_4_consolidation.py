...
class V154ConsolidationTest(unittest.TestCase):
    ...
    def test_docs_make_clerk_optional_and_project_read_only(self):
        ...
        self.assertIn("delegate Discovery first", skill)
        ...
        self.assertIn("Do not spend parent context predicting the diff", skill)
        self.assertIn("Use `write_paths` only when the governing authority itself already imposes", skill)
