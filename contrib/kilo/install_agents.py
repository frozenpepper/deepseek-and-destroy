...
def install_set(contrib_root: Path, target: Path, model: str) -> list[dict[str, object]]:
    template_dir = contrib_root / "agents"
...
def main() -> int:
    ...
    parser.add_argument("--contrib-root", type=Path, default=Path(__file__).resolve().parent)
    ...
    contrib = args.contrib_root.resolve()
    try:
        if not args.skip_model_verify:
            verify_model(args.model)
        entries = [
            {**e, "scope": "project"}
            for e in install_set(contrib, project / ".kilo" / "agents", args.model)
        ]
        if args.global_install:
            entries.extend(
                {**e, "scope": "global"}
                for e in install_set(contrib, Path.home() / ".config" / "kilo" / "agents", args.model)
            )
        result = {
            "model": args.model,
