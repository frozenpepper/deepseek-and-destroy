...
2. Otherwise launch `run_worker.py --detach`, then invoke:
   ...
   python3 <skill-root>/scripts/wait_worker.py \
     ...
     --timeout <longest-safe-host-timeout>
   ```

3. If the wait returns status `timeout`, that is a **non-event**. Emit no prose,
   inspect no logs/state/repository, and immediately run the same long wait again.
