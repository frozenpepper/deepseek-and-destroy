...
def main() -> int:
    ...
    lines = [
        f"# Task {args.task_id} — {unit}",
        f"Contract revision: r{args.revision:04d}",
        "",
        "## Objective",
    ...
    if authorities:
        lines += ["## Authority", *[f"- `{p}`" for p in authorities], ""]
    if inputs:
        lines += ["## Inputs", *[f"- `{p}`" for p in inputs], ""]
    if args.expected_scope or args.excluded or mechanical:
        lines += ["## Scope"]
        if args.expected_scope:
            lines += ["Expected:", *bullet_lines(args.expected_scope)]
        if args.excluded:
            lines += ["Excluded:", *bullet_lines(args.excluded)]
        if mechanical:
            lines += ["Mechanical facts (current-contract identity):", *mechanical_lines]
        lines += [""]
    lines += [
    ...
    if args.risk:
        lines += ["## Risk hypotheses", *[f"{i}. {clean(v)}" for i, v in enumerate(args.risk, 1)], ""]
    lines += [
        "## Acceptance criteria",
    ...
    if args.proof:
        lines += ["## Proof Obligations", *bullet_lines(args.proof), ""]
    if args.verification:
        lines += ["## Verification", *bullet_lines(args.verification), ""]
    if args.clerk_check:
        lines += ["## Evidence Clerk Checks", *bullet_lines(args.clerk_check), ""]
    task_outputs = [clean(x) for x in args.task_output if clean(x)]
    if progress:
        task_outputs.append(f"Maintain configured progress/documentation file when role-authorized: `{progress}`")
    if task_outputs:
        lines += ["## Task outputs", *[f"- {x}" for x in task_outputs], ""]
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text("\n".join(lines), encoding="utf-8")
    digest = sha256(output)
    print(json.dumps({
        "format": "dsd-task-contract-v3",
